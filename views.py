...
def teacher_dashboard(request, pk):
    """Dashboard for teacher -- pk is the User ID."""
    try:
    ...
    for classroom in teacher.classes.all():
        active_session = classroom.sessions.filter(
            active=True, expires_at__gte=now
        ...
        present_today   = all_records.filter(timestamp__date=now.date()).count()
        present_weekly  = all_records.filter(timestamp__date__gte=start_of_week.date()).count()
        present_monthly = all_records.filter(timestamp__date__gte=start_of_month.date()).count()

        # Total sessions held this month for this classroom
        total_sessions_month = AttendanceSession.objects.filter(
            classroom=classroom,
            start_time__date__gte=start_of_month.date()
        ).count()

        # Per-student monthly attendance table for this subject
        student_records = []
        for s in classroom.students.all():
            s_monthly = all_records.filter(
                student=s,
                timestamp__date__gte=start_of_month.date()
            ).count()
            s_total = all_records.filter(student=s).count()
            pct = round((s_monthly / total_sessions_month) * 100) if total_sessions_month > 0 else 0
